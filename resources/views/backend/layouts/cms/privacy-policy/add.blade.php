        ...
        <div class="col-md-8 py-5 mx-auto">
            <div class="card card-primary card-outline">
                <div class="card-header text-center">
                    {{ isset($webPrivacyPolicy) ? 'Edit Privacy Policy' : 'Add Privacy Policy' }}
                </div>
                <div class="card-body">
                    ...
                    <form action="{{ isset($webPrivacyPolicy) ? route('web-privacy-policies.update', $webPrivacyPolicy->id) : route('web-privacy-policies.store') }}" method="POST">
                        @csrf
                        @if(isset($webPrivacyPolicy)) @method('PUT') @endif
                        ...
                        <div class="form-group">
                            ...
                            <div id="sub-items">
                                @if(old('sub_title'))
                                    ...
                                    @endforeach
                                @elseif(isset($webPrivacyPolicy))
                                    @foreach($webPrivacyPolicy->sub_title as $i => $st)
                                        <div class="sub-item mb-2">
                                            <input type="text" name="sub_title[]" class="form-control mb-1" value="{{ $st }}" placeholder="Sub Title" required>
                                            <textarea name="description[]" class="form-control" placeholder="Description">{{ $webPrivacyPolicy->description[$i] ?? '' }}</textarea>
                                            <button type="button" class="btn btn-sm btn-danger mt-1" onclick="removeItem(this)">Remove</button>
                                        </div>
                                    @endforeach
                                @else
                        ...
                        <button type="submit" class="btn btn-success mt-3">{{ isset($webPrivacyPolicy) ? 'Update' : 'Save' }} Policy</button>
                    </form>
                </div>
            </div>
        </div>
